...
export function demoOpen(instrument, direction, lots, entryPrice, stopLoss, takeProfit, meta = {}) {
  // Estimasi margin (leverage 100:1): 0.01 lot = 1000 unit = $10 margin untuk EUR/USD
  // Untuk XAU: 0.01 lot = 1 oz * $2320 / 100 leverage = $23.2 margin
  const isXAU = instrument === 'XAU_USD';
  const isXAG = instrument === 'XAG_USD';
  const basePrice = isXAU ? 2320 : isXAG ? 27 : 1;
  const marginRequired = lots * basePrice * (isXAU || isXAG ? 10 : 1000) / 100;
  if (demoState.usdBalance < marginRequired) return { success: false, error: `Saldo tidak cukup (butuh $${marginRequired.toFixed(2)} margin)` };
...
export function demoClose(positionId, closePrice, reason = 'manual') {
  ...
  // Pip value berbeda per instrument
  const pip = pos.instrument === 'XAU_USD' ? 0.01
            : pos.instrument === 'XAG_USD' ? 0.001
            : pos.instrument?.includes('JPY') ? 0.01
            : 0.0001;
  const pnlPips = isBuy
    ? (closePrice - pos.entryPrice) / pip
    : (pos.entryPrice - closePrice) / pip;
  // Pip value USD: XAU $0.01/pip/micro, JPY ~$0.093/pip/micro, lainnya $0.10/pip/micro
  const pipValueUSD = pos.instrument === 'XAU_USD' ? pos.lots * 1.0
                    : pos.instrument === 'XAG_USD' ? pos.lots * 5.0
                    : pos.lots * (pos.instrument?.includes('JPY') ? 9.30 : 10.0);
  const pnlUSD = parseFloat((pnlPips * pipValueUSD * 0.01).toFixed(2));
...
export function updatePositions(instrument, currentPrice) {
  demoState.openPositions = demoState.openPositions.map(pos => {
    ...
    const isBuy  = pos.direction === 'buy';
    const pip = instrument === 'XAU_USD' ? 0.01
              : instrument === 'XAG_USD' ? 0.001
              : instrument?.includes('JPY') ? 0.01
              : 0.0001;
    const pnlPips = isBuy
      ? (currentPrice - pos.entryPrice) / pip
      : (pos.entryPrice - currentPrice) / pip;
    const pipValueUSD = instrument === 'XAU_USD' ? pos.lots * 1.0
                      : instrument === 'XAG_USD' ? pos.lots * 5.0
                      : pos.lots * (instrument?.includes('JPY') ? 9.30 : 10.0);

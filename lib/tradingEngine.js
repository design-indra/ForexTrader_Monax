...
function getAdvancedContext(candles, instrument = 'EUR_USD') {
  const closes = candles.map(c => c.close);
  const close  = closes[closes.length - 1];
  // SR threshold: persentase dari harga (0.05% untuk forex biasa, 0.1% untuk XAU/metal)
  const isMetals = ['XAU_USD','XAG_USD'].includes(instrument || '');
  const srThreshold = isMetals ? 0.001 : 0.0005;
  const sr         = detectSupportResistance(candles, 20, srThreshold);
...
// ─── Level 1: Scalper ─────────────────────────────────────────────────────────
function level1Signal(candles, direction = 'both', instrument = 'EUR_USD') {
  const closes  = candles.map(c => c.close);
  ...
  const ctx      = getAdvancedContext(candles, instrument);
...
function level2Signal(candles, direction = 'both', instrument = 'EUR_USD') {
  const base   = level1Signal(candles, direction, instrument);
  const closes = candles.map(c => c.close);
...
// ─── Level 3: AI Scoring ──────────────────────────────────────────────────────
function level3Signal(candles, direction = 'both', instrument = 'EUR_USD') {
  const closes  = candles.map(c => c.close);
  const volumes = candles.map(c => c.volume);
...
function level4Signal(candles, direction = 'both', instrument = 'EUR_USD') {
  const base  = level3Signal(candles, direction, instrument);
  const closes = candles.map(c => c.close);
  const ctx   = base.context;
...
function level5Signal(candles, direction = 'both', openPositions = [], instrument = 'EUR_USD') {
  const base   = level4Signal(candles, direction, instrument);
  const ctx    = base.context;
  const closes = candles.map(c => c.close);
  const close  = closes[closes.length - 1];
...
export async function runCycle(candles, currentState = {}) {
  ...
  try {
    switch (botState.level) {
      case 1: signal = level1Signal(candles, direction, instrument); break;
      case 2: signal = level2Signal(candles, direction, instrument); break;
      case 3: signal = level3Signal(candles, direction, instrument); break;
      case 4: signal = level4Signal(candles, direction, instrument); break;
      case 5: signal = level5Signal(candles, direction, openPositions, instrument); break;
      default: signal = level1Signal(candles, direction, instrument);
    }
  ...
  if (allowed && (signal.action === 'BUY' || signal.action === 'SELL') && openPositions.length === 0) {
    if (Date.now() - botState.lastActionTime < cooldownMs) {
      ...
      if (sizing.lots <= 0) {
        addLog(`⚠️ Saldo tidak cukup (balance: ${balance.toFixed(2)} USD)`, 'warning');
      } else {

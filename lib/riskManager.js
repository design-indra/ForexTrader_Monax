...
export function calculateLotSize(balance, slPips, instrument = 'EUR_USD', botState = {}, signalGrade = 'C') {
  ...
  // EUR/USD, GBP/USD dll: $0.10 per pip
  // JPY pairs: ~$0.0932 per pip
  // XAU/USD: pip=$0.01, 1 lot=100oz, pip value=$1/lot = $0.01/micro lot
  // XAG/USD: pip=$0.001, pip value=$0.05/micro lot
  const pipValuePerMicroLot = instrument === 'XAU_USD' ? 1.0
    : instrument === 'XAG_USD' ? 0.50
    : instrument.includes('JPY') ? 0.0932
    : 0.10; // USD per pip per 0.01 lot
  ...
  const lots = parseFloat((Math.floor(microLots * 1000) / 1000).toFixed(3));
  const maxLots = s.defaultLotSize || 0.01;
